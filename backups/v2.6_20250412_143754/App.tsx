  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        ...
        // Intentar cargar template desde otra ubicación
        try {
          // Intentar desde public/templates
          const fallbackUrl = `/templates/${templateId}.css`;
          console.log(`Intentando cargar template desde: ${fallbackUrl}`);
          
          const fallbackResponse = await fetch(fallbackUrl);
          
          if (!fallbackResponse.ok) {
            throw new Error(`Fallback template not found: ${templateId}`);
          }
          
          const fallbackCssContent = await fallbackResponse.text();
          
          if (!fallbackCssContent || fallbackCssContent.trim().length === 0) {
            throw new Error(`Fallback template is empty: ${templateId}`);
          }
          
          setCSS(fallbackCssContent);
          console.log(`Template ${templateId} cargado con éxito desde ubicación alternativa`);
        } catch (fallbackError) {
          console.error('Error loading fallback template:', fallbackError);
          // Usar CSS embebido como última opción
          console.log('Usando CSS embebido por defecto');
          setCSS(`
            body { 
              font-family: Arial, sans-serif; 
              line-height: 1.6;
