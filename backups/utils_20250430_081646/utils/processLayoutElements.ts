export interface DataAttributes {
  animation?: 'fade' | 'slide' | 'glow' | 'shake';
...
function processAttributes(attributeString: string): {
  classes: string[];
  styles: Record<string, string>;
  dataAttributes: DataAttributes;
} {
  ...
  if (dataMatches) {
    dataMatches.forEach(match => {
      ...
      const cleanKey = key.replace('data-', '') as keyof DataAttributes;
...
export function processLayoutElements(content: string): string {
  const cache = new Map<string, string>();

  function processPanel(panel: string, cache: Map<string, string>): string {
...
function extractAttributes(element: string): Record<string, string> {
...
function validatePanelStructure(panel: string): ValidatedPanel {
...
export function processPanel(panel: string): ProcessedPanel {
